...
public class TilesetEditorDialog extends JDialog {
    ...
    public TilesetEditorDialog(Window owner) {
        ...
        //Palette folder assignment for the current tile selection (the range
        //selection of this editor: left click start, right click end)
        jbMoveToFolder = new JButton("Assign to Folder...");
        jbMoveToFolder.setToolTipText("Assign the selected tiles to a palette folder of the main "
                + "window tile list. Folders never change the tile order.");
        jbMoveToFolder.addActionListener(e -> moveSelectedTilesToFolder());

        jbRemoveFromFolder = new JButton("Remove From Folder");
        jbRemoveFromFolder.addActionListener(e -> removeSelectedTilesFromFolder());

        jbCreateSmartDrawing = new JButton("Create Smart Drawing...");
        jbCreateSmartDrawing.setToolTipText("Create a Smart Drawing template from the selected "
                + "tiles and their positions in a palette folder.");
        jbCreateSmartDrawing.addActionListener(e -> createSmartDrawingFromFolderSelection());

        JPanel folderActions = new JPanel(new MigLayout("insets 5 0 2 0,hidemode 3,gap 5 0",
                "[grow,fill][grow,fill][grow,fill]", "[fill]"));
        folderActions.add(jbMoveToFolder, "cell 0 0");
        folderActions.add(jbRemoveFromFolder, "cell 1 0");
        folderActions.add(jbCreateSmartDrawing, "cell 2 0");
        ((JPanel) jbAddTile.getParent()).add(folderActions, "cell 0 1,span 4,growx");
    }

    private void createSmartDrawingFromFolderSelection() {
        String folderPath = tileSelector.getSelectedLayoutFolderPath();
        ArrayList<Integer> indices = folderPath == null ? new ArrayList<>()
                : tileSelector.getSelectedIndicesInFolder(folderPath);
        if (indices.size() < 2) {
            JOptionPane.showMessageDialog(this,
                    "Select at least two tiles in the Tile Selector first.\n"
                            + "Left click the first tile and right click the last tile.",
                    "Create Smart Drawing", JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        PaletteFolder folder = handler.getTileset().getPaletteFolder(folderPath);
        if (folderPath == null || folderPath.isEmpty() || folder == null || folder.getColumns() <= 0) {
            showSmartDrawingLayoutError("The selected tiles must be placed in the same folder grid.");
            return;
        }
        ...
        for (int index : indices) {
            ...
            if (!tile.isInPaletteFolder(folderPath) || tile.getPaletteSlot(folderPath) < 0) {
                showSmartDrawingLayoutError(
                        "Every selected tile must have a fixed cell in the same folder grid.");
                return;
            }
            if (!tile.isSizeOne()) {
                showSmartDrawingLayoutError("Smart Drawing templates only support 1 x 1 tiles.");
                return;
            }
            int col = tile.getPaletteSlot(folderPath) % folder.getColumns();
            int row = tile.getPaletteSlot(folderPath) / folder.getColumns();
        ...
        if (maxCol - minCol + 1 > SmartGrid.width || maxRow - minRow + 1 > SmartGrid.height) {
            showSmartDrawingLayoutError("The selected layout must fit inside a 5 x 3 Smart Drawing grid.");
            return;
        }

        SmartGridEditable grid = new SmartGridEditable();
        grid.setPaletteFolder(folderPath);
        for (int index : indices) {
            Tile tile = handler.getTileset().get(index);
            int x = tile.getPaletteSlot(folderPath) % folder.getColumns() - minCol;
            int y = tile.getPaletteSlot(folderPath) / folder.getColumns() - minRow;
            if (y == 2 && x >= 3) {
                showSmartDrawingLayoutError(
                        "The bottom-right two cells are reserved by the Smart Drawing selector.");
                return;
            }
            grid.sgrid[x][y] = tile;
        }

        int insertIndex = smartGridEditableDisplay.getSmartGridArray().size();
        for (int i = 0; i < smartGridEditableDisplay.getSmartGridArray().size(); i++) {
            if (folderPath.equals(smartGridEditableDisplay.getSmartGridArray().get(i).getPaletteFolder())) {
                insertIndex = i + 1;
            }
        }
        smartGridEditableDisplay.getSmartGridArray().add(insertIndex, grid);
        handler.setSmartGridIndexSelected(insertIndex);
        smartGridEditableDisplay.updateSize();
        smartGridEditableDisplay.revalidate();
        smartGridEditableDisplay.repaint();
        jScrollPaneSmartGrid.getVerticalScrollBar().setValue(
                jScrollPaneSmartGrid.getVerticalScrollBar().getMaximum());
    }

    private void showSmartDrawingLayoutError(String message) {
        JOptionPane.showMessageDialog(this, message, "Create Smart Drawing",
                JOptionPane.WARNING_MESSAGE);
    }

    private void removeSelectedTilesFromFolder() {
        String folderPath = tileSelector.getSelectedLayoutFolderPath();
        if (folderPath == null) {
            JOptionPane.showMessageDialog(this, "Select tiles in a named folder first.",
                    "Remove From Folder", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ArrayList<Integer> indices = tileSelector.getSelectedIndicesInFolder(folderPath);
        for (int index : indices) {
            handler.getTileset().get(index).removePaletteFolder(folderPath);
        }
        refreshPaletteFolderViews();
    }

    private void moveSelectedTilesToFolder() {
        ArrayList<Integer> indices = tileSelector.getIndicesSelected();
        if (indices.isEmpty() || handler.getTileset().size() == 0) {
            return;
        }
        ArrayList<String> options = new ArrayList<>();
        options.add("(New Folder...)");
        options.add("All Tiles only");
        for (tileset.PaletteFolder folder : handler.getTileset().getPaletteFolders()) {
            if (!folder.getPath().isEmpty()) {
                options.add(folder.getPath());
            }
        }
        String choice = (String) JOptionPane.showInputDialog(this,
                "Assign " + indices.size() + " tile(s) to folder:", "Assign to Folder",
                JOptionPane.PLAIN_MESSAGE, null, options.toArray(), options.get(options.size() - 1));
        if (choice == null) {
            return;
        }
        String path;
        if (choice.equals(options.get(0))) {
            String input = JOptionPane.showInputDialog(this,
                    "Folder path (use \"/\" for subfolders, e.g. \"Terrain/HGSS Grass\"):",
                    "New Folder",
                    JOptionPane.PLAIN_MESSAGE);
            if (input == null) {
                return;
            }
            path = sanitizeFolderPath(input);
            if (path.isEmpty()) {
                return;
            }
            handler.getTileset().getOrCreatePaletteFolderWithParents(path);
        } else if (choice.equals(options.get(1))) {
            path = tileset.PaletteFolder.UNSORTED;
        } else {
            path = choice;
        }
        for (int index : indices) {
            tileset.Tile tile = handler.getTileset().get(index);
            tile.setPaletteFolder(path);
            tile.setPaletteSlot(-1);
        }
        refreshPaletteFolderViews();
    }

    private String sanitizeFolderPath(String input) {
    ...
    private void refreshPaletteFolderViews() {
    ...
    private void fixSmartGridIndices() {
        ...
        for (SmartGridEditable smartGrid : smartGridEditableArray) {
            SmartGrid grid = new SmartGrid(smartGrid.toTileIndices(handler.getTileset()));
            grid.setPaletteFolder(smartGrid.getPaletteFolder());
            smartGridArray.add(grid);
        }
