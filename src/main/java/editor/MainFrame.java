...
public class MainFrame extends JFrame {
    ...
    //Custom components (not generated by JFormDesigner)
    private JToggleButton jtbModeSelect;
    private JToggleButton jtbModeLasso;
    private JToggleButton jtbModeWand;
    private JToggleButton jtbModeMoveSelect;
    private JToggleButton jtbModeBucket;
    private JToggleButton jtbModePicker;
    ...
    private JButton jbDeleteSelection;
    private JMenuItem jmiSelectAll;
    private JMenuItem jmiCopySelection;
    private JMenuItem jmiCutSelection;
    private JMenuItem jmiPasteSelection;
    ...
    /**
     * Components added by hand (not generated by JFormDesigner): the region
     * selection tool button, its Edit menu entries and the cursor coordinates
     * readout in the status bar.
     */
    private void initCustomComponents() {
        ButtonGroup drawModeGroup = ((DefaultButtonModel) jtbModeEdit.getModel()).getGroup();

        //Paint.net style tool buttons
        jtbModeSelect = createToolToggle(new ImageIcon(createSelectModeIcon()),
                "Rectangle Select (M) - drag borders to resize, right click inside for actions",
                MapDisplay.EditMode.MODE_SELECT, drawModeGroup);
        jtbModeLasso = createToolToggle(new ImageIcon(createLassoIcon()),
                "Lasso Select - draw an outline, it gets filled on release (Ctrl adds to selection)",
                MapDisplay.EditMode.MODE_SELECT_LASSO, drawModeGroup);
        jtbModeWand = createToolToggle(new ImageIcon(createWandIcon()),
                "Magic Wand - select connected tiles of the same type (Ctrl adds / removes, "
                        + "Shift adds all matching tiles in the map)",
                MapDisplay.EditMode.MODE_SELECT_WAND, drawModeGroup);
        jtbModeMoveSelect = createToolToggle(new ImageIcon(createMoveSelectIcon()),
                "Move Selection - drag the selected tiles to a new place (Esc cancels)",
                MapDisplay.EditMode.MODE_MOVE_SELECT, drawModeGroup);
        jtbModeBucket = createToolToggle(centeredIcon("/cursors/floodFillIcon.png"),
                "Paint Bucket - flood fill with the selected tile (stays inside a selection)",
                MapDisplay.EditMode.MODE_BUCKET, drawModeGroup);
        jtbModePicker = createToolToggle(centeredIcon("/cursors/grabColorCursor.png"),
                "Tile Picker - click a tile on the map to select it in the tileset",
                MapDisplay.EditMode.MODE_PICKER, drawModeGroup);
        jtbModeLine = createToolToggle(new ImageIcon(createLineIcon()),
                "Draw Line - drag to draw a straight line of tiles",
                MapDisplay.EditMode.MODE_LINE, drawModeGroup);
        jtbModeRectShape = createToolToggle(new ImageIcon(createRectShapeIcon()),
                "Draw Rectangle - drag to draw a rectangle outline of tiles",
                MapDisplay.EditMode.MODE_SHAPE_RECT, drawModeGroup);
        jtbModeEllipseShape = createToolToggle(new ImageIcon(createEllipseShapeIcon()),
        ...
        //Keep the toolbar at its preferred height (top aligned) instead of
        //stretching the button grid over the whole panel
        GroupLayout jpToolsLayout = (GroupLayout) jpTools.getLayout();
        jpToolsLayout.setVerticalGroup(jpToolsLayout.createSequentialGroup()
                .addComponent(jtTools, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
                .addGap(0, 0, Short.MAX_VALUE));
        //Bigger, stacked layer arrows
        jbMoveLayerUp.setIcon(scaledIcon("/icons/upIcon.png", 20));
        jbMoveLayerDown.setIcon(scaledIcon("/icons/downIcon.png", 20));

        jbCopySelection = createActionButton(scaledIcon("/icons/CopyIcon.png", 24),
                "Copy Selection (Ctrl+C)", () -> mapDisplay.copySelection());
        jbCutSelection = createActionButton(new ImageIcon(createScissorsIcon()),
                "Cut Selection (Ctrl+X)", () -> mapDisplay.cutSelection());
        jbPasteSelection = createActionButton(scaledIcon("/icons/pasteIcon.png", 24),
        ...
        jmiSelectAll.addActionListener(e -> mapDisplay.selectAllInSelectedMap());

        jmiCopySelection = new JMenuItem("Copy Selection");
        jmiCopySelection.setIcon(new ImageIcon(getClass().getResource("/icons/CopyIcon.png")));
        jmiCopySelection.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.CTRL_DOWN_MASK));
        jmiCopySelection.addActionListener(e -> mapDisplay.copySelection());

        jmiCutSelection = new JMenuItem("Cut Selection");
        jmiCutSelection.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.CTRL_DOWN_MASK));
        jmiCutSelection.addActionListener(e -> mapDisplay.cutSelection());

        jmiPasteSelection = new JMenuItem("Paste Selection");
        jmiPasteSelection.setIcon(new ImageIcon(getClass().getResource("/icons/pasteIcon.png")));
        jmiPasteSelection.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_V, InputEvent.CTRL_DOWN_MASK));
        jmiPasteSelection.addActionListener(e -> mapDisplay.startPaste());

        jmiDeleteSelection = new JMenuItem("Delete Selection");
        jmiDeleteSelection.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0));
        jmiDeleteSelection.addActionListener(e -> mapDisplay.deleteSelection());

        jmiFillSelection = new JMenuItem("Fill Selection with Tile");
        jmiFillSelection.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F, InputEvent.CTRL_DOWN_MASK));
        jmiFillSelection.addActionListener(e -> mapDisplay.fillSelection());
        ...
        jmEdit.insert(jmiSelectAll, 11);
        jmEdit.insert(jmiCopySelection, 12);
        jmEdit.insert(jmiCutSelection, 13);
        jmEdit.insert(jmiPasteSelection, 14);
        ...
        jLabel6.setText("Map Coordinates:");
        jlCursorCoordsTitle = new JLabel("Tile Coordinates:");
        jlCursorCoords = new JLabel(" ");
        jlCursorCoords.setPreferredSize(new Dimension(60, 14));
        ...
        int fittedWidth = Math.min(getWidth(), usable.width);
        int fittedHeight = Math.min(getHeight(), usable.height);
    ...
    private void addToolGroup(int columns, AbstractButton... buttons) {
        ...
        JPanel panel = new JPanel(new GridLayout(0, columns, 2, 2));
        panel.setOpaque(false);
        for (AbstractButton b : buttons) {
            //Compact 24px icons keep the whole tools column on screen
            Icon icon = b.getIcon();
            if (icon instanceof ImageIcon && (icon.getIconWidth() > 24 || icon.getIconHeight() > 24)) {
                b.setIcon(new ImageIcon(((ImageIcon) icon).getImage()
                        .getScaledInstance(24, 24, Image.SCALE_SMOOTH)));
            }
            b.setMargin(new Insets(1, 1, 1, 1));
            applyToolButtonBackground(b);
            panel.add(b);
        }
        panel.setAlignmentX(0.0f);
        Dimension pref = panel.getPreferredSize();
        //All groups share the width of the first (two column) group
        if (toolGroupWidth == 0) {
            toolGroupWidth = pref.width;
        } else {
    ...
    /** Icon recentered inside a 32x32 canvas based on its opaque bounding box. */
    private ImageIcon centeredIcon(String resourcePath) {
        try {
            ...
            int w = maxX - minX + 1;
            int h = maxY - minY + 1;
            java.awt.image.BufferedImage out = newIconImage();
            Graphics2D g = out.createGraphics();
            g.drawImage(src.getSubimage(minX, minY, w, h), (32 - w) / 2, (32 - h) / 2, null);
            g.dispose();
    ...
    /** Icon smoothly scaled to the given square size. */
    private ImageIcon scaledIcon(String resourcePath, int size) {
        Image img = new ImageIcon(getClass().getResource(resourcePath)).getImage();
        return new ImageIcon(img.getScaledInstance(size, size, Image.SCALE_SMOOTH));
    }

    private JButton createActionButton(ImageIcon icon, String toolTip, Runnable action) {
    ...
    /** Slightly darker background so unselected tool buttons stand out from the panel. */
    private static void applyToolButtonBackground(AbstractButton button) {
    ...
    private JToggleButton createToolToggle(ImageIcon icon, String toolTip,
                                           MapDisplay.EditMode mode, ButtonGroup group) {
        ...
        button.addActionListener(e -> mapDisplay.setEditMode(mode));
        if (group != null) {
            group.add(button);
        }
        return button;
    ...
    private static java.awt.image.BufferedImage newIconImage() {
        return new java.awt.image.BufferedImage(32, 32, java.awt.image.BufferedImage.TYPE_INT_ARGB);
    }

    private static Graphics2D iconGraphics(java.awt.image.BufferedImage img) {
    ...
    private static final Stroke ICON_DASHED = new BasicStroke(2, BasicStroke.CAP_BUTT,
            BasicStroke.JOIN_MITER, 10.0f, new float[]{4.0f, 3.0f}, 0.0f);
    private static final Color ICON_DARK = new Color(45, 45, 45);
    private static final Color ICON_LIGHT = new Color(225, 225, 225);

    private static java.awt.image.BufferedImage createSelectModeIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = img.createGraphics();
        g.setStroke(ICON_DASHED);
        g.setColor(ICON_DARK);
        g.drawRect(7, 8, 20, 18);
        g.setColor(ICON_LIGHT);
        g.drawRect(5, 6, 20, 18);
        g.dispose();
        return img;
    }

    private static java.awt.image.BufferedImage createLassoIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(ICON_DASHED);
        int[] xs = {6, 14, 24, 27, 22, 12, 5};
        int[] ys = {14, 6, 8, 17, 25, 26, 21};
        java.awt.Polygon blob = new java.awt.Polygon(xs, ys, xs.length);
        g.setColor(ICON_DARK);
        g.translate(1, 1);
        g.drawPolygon(blob);
        g.translate(-2, -2);
    ...
    private static java.awt.image.BufferedImage createWandIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(new BasicStroke(3, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g.setColor(new Color(150, 100, 40));
        g.drawLine(10, 22, 24, 8);
    ...
    private static java.awt.image.BufferedImage createMoveSelectIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(ICON_DASHED);
        g.setColor(ICON_LIGHT);
    ...
    private static java.awt.image.BufferedImage createLineIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(new BasicStroke(3, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g.setColor(ICON_DARK);
        g.drawLine(7, 26, 26, 7);
        g.setColor(ICON_LIGHT);
        g.drawLine(6, 25, 25, 6);
        g.dispose();
        return img;
    }

    private static java.awt.image.BufferedImage createRectShapeIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(new BasicStroke(3));
        g.setColor(ICON_DARK);
        g.drawRect(7, 10, 19, 15);
        g.setColor(ICON_LIGHT);
        g.drawRect(6, 9, 19, 15);
        g.dispose();
        return img;
    }

    private static java.awt.image.BufferedImage createEllipseShapeIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(new BasicStroke(3));
        g.setColor(ICON_DARK);
        g.drawOval(7, 10, 19, 15);
        g.setColor(ICON_LIGHT);
        g.drawOval(6, 9, 19, 15);
        g.dispose();
        return img;
    }

    private static java.awt.image.BufferedImage createScissorsIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        //Open silver blades forming an X with a pivot dot and red handle rings
        g.setStroke(new BasicStroke(3.2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g.setColor(new Color(218, 223, 230));
        g.drawLine(8, 3, 21, 19);
        g.drawLine(24, 3, 11, 19);
        g.setStroke(new BasicStroke(2.8f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g.setColor(new Color(226, 88, 88));
        g.drawOval(15, 19, 10, 10);
        g.drawOval(7, 19, 10, 10);
        g.setColor(new Color(140, 145, 155));
        g.fillOval(14, 10, 5, 5);
        g.dispose();
        return img;
    }

    private static java.awt.image.BufferedImage createTrashIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(new BasicStroke(2, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g.setColor(ICON_LIGHT);
        ...
        g.drawLine(12, 10, 13, 7);
        g.drawLine(20, 10, 19, 7);
        g.drawLine(13, 7, 19, 7);
        //Can
        g.drawRect(10, 12, 12, 14);
        //Vertical slats
        g.drawLine(14, 14, 14, 24);
        g.drawLine(18, 14, 18, 24);
        g.setColor(new Color(220, 90, 90));
        g.drawLine(11, 13, 21, 13);
        g.dispose();
        return img;
    }

    /** Dashed marquee with a red "no" symbol over its corner. */
    private static java.awt.image.BufferedImage createDeselectIcon() {
        java.awt.image.BufferedImage img = newIconImage();
        Graphics2D g = iconGraphics(img);
        g.setStroke(ICON_DASHED);
        g.setColor(ICON_LIGHT);
        g.drawRect(4, 5, 17, 15);
        g.setStroke(new BasicStroke(2.6f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g.setColor(new Color(232, 82, 82));
        g.drawOval(16, 14, 13, 13);
        g.drawLine(18, 16, 27, 25);
        g.dispose();
        return img;
    }

    public JToggleButton getJtbModeSelect() {
        return jtbModeSelect;
    }

    public JToggleButton getJtbModeMoveSelect() {
        return jtbModeMoveSelect;
    }

    /** Enables or disables the tools that only work in the top-down (ortho) view. */
    public void setOrthoToolsEnabled(boolean enabled) {
        jtbModeClear.setEnabled(enabled);
        jtbModeSmartPaint.setEnabled(enabled);
        jtbModeInvSmartPaint.setEnabled(enabled);
        ...
        jtbModeBucket.setEnabled(enabled);
        jtbModePicker.setEnabled(enabled);
    ...
    public void updateCursorTileCoords(Point tile) {
        jlCursorCoords.setText("(" + tile.x + ", " + tile.y + ")");
    }
    ...
    public void updateTileSelectorLayout() {
        viewUpdater.updateTileSelectorLayout();
    }
    ...
    public void renderTileThumbnail(int tileIndex) {
        viewUpdater.renderTileThumbnail(tileIndex);
    }
